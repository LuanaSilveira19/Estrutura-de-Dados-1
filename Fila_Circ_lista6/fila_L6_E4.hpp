...
struct Fila
{
    ...
    Impressao *dados;//ponteiro para o vetor que será alocado para armazenar os dados
...
void inicializaF(Fila *f, int tam) /// inicialização da fila
{
    ...
    f->fim = 0;
    f->total = 0;
    f->dados = new Impressao[tam];/// aloca memória para vetor
}
...
bool buscaF(Fila *f, string nome, int tam)
{
    ...
    {
        //desenfilera F para a variável doc
        doc=desenfileiraF(f);
